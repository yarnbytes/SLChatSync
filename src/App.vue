  ...
  <div class="app-container">
    ...
    <div class="title-bar" style="-webkit-app-region: drag;">
      ...
      <div class="title-bar-right" style="-webkit-app-region: no-drag;">
        ...
        <button class="title-btn" @click="toggleLanguage" title="切换语言 / Language" style="font-size: 11px; font-weight: bold;">
          {{ locale === 'zh' ? 'EN' : 'ZH' }}
        </button>
        <button class="title-btn" @click="handleMin">
          <svg viewBox="0 0 16 16"><path d="M14 8v1H2V8h12z" fill="currentColor"/></svg>
        </button>
        <button class="title-btn close-btn" @click="handleClose">
          <svg viewBox="0 0 16 16"><path d="M14 3.41L12.59 2 8 6.59 3.41 2 2 3.41 6.59 8 2 12.59 3.41 14 8 9.41 12.59 14 14 12.59 9.41 8 14 3.41z" fill="currentColor"/></svg>
        </button>
      </div>
    </div>
        ...
        <t-row :gutter="24" class="main-row">
          ...
          <t-col :span="6" class="form-column">
            <t-form :data="config" layout="vertical" labelAlign="top" class="config-form">
              <t-form-item :label="$t('app.form.pathLabel')" name="appDataPath">
                <template #label><Folder class="w-4 h-4 mr-1" /> {{ $t('app.form.pathLabel') }}</template>
                <t-input v-model="config.appDataPath" :placeholder="$t('app.form.pathPlaceholder')" @change="handlePathChange" />
              </t-form-item>

              <t-form-item :label="$t('app.form.folderLabel')" name="accountName">
                <template #label><User class="w-4 h-4 mr-1" /> {{ $t('app.form.folderHelp') }}</template>
                <t-select v-model="config.accountName" :placeholder="$t('app.form.folderPlaceholder')" @change="validateInputs" :options="availableAccounts.map(a => ({ label: a, value: a }))">
                </t-select>
              </t-form-item>

              <t-form-item :label="$t('app.form.repoLabel')" name="repoUrl">
                <template #label><Github class="w-4 h-4 mr-1" /> {{ $t('app.form.repoHelp') }}</template>
                <t-input v-model="config.repoUrl" :placeholder="$t('app.form.repoPlaceholder')" @change="validateInputs" />
              </t-form-item>

              <t-form-item :label="$t('app.form.userLabel')" name="gitUsername">
                <template #label><User class="w-4 h-4 mr-1" /> {{ $t('app.form.userLabel') }}</template>
                <t-input v-model="config.gitUsername" :placeholder="$t('app.form.userPlaceholder')" @change="validateInputs" />
              </t-form-item>

              <t-form-item :label="$t('app.form.tokenLabel')" name="gitToken">
                <template #label><KeyRound class="w-4 h-4 mr-1" /> {{ $t('app.form.tokenLabel') }}</template>
                <t-input type="password" v-model="config.gitToken" :placeholder="$t('app.form.tokenPlaceholder')" @change="validateInputs" />
              </t-form-item>
              ...
              <t-form-item :label="$t('app.form.encryptLabel')" name="encryptPassword">
                ...
                <t-input type="password" v-model="config.encryptPassword" :placeholder="$t('app.form.encryptPlaceholder')" @change="validateInputs" />
              </t-form-item>
            </t-form>
          </t-col>
          ...
          <t-col :span="6" class="action-column">
            ...
            <t-button 
              ...
              class="sync-btn"
            >
              <template #icon><CloudSync v-if="!isSyncing" /></template>
...
<style>
  ...
  .title-btn svg { width: 14px; height: 14px; }
  .title-btn:hover { background: #e5e7eb; }
  ...
  .header-section h2 { margin: 0; font-size: 20px; color: #111827; }
  ...
  .mr-1 { display: inline-block; vertical-align: middle; margin-right: 4px; }

  .config-form .t-form__label { font-size: 13px; color: #4b5563; }
